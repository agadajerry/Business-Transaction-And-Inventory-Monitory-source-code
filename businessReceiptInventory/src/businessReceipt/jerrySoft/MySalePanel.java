...
public class MySalePanel extends JDialog{
	...
	public JTextField customerField, customerAddrField;
...
private void initUI() {
	...
	URL urlImage = MySalePanel.class.getResource("./sbg image/loginSuccessful.JPG");
...
public void customerLastRow() {
	...
	try {
		ps = DbConnection.getConnection().prepareStatement(lastRow);
...
public void productAndVendorList() {
	...
	try {
		...
		ps = DbConnection.getConnection().prepareStatement(vendorQuery);
		rs = ps.executeQuery();
		pre = DbConnection.getConnection().prepareStatement(productQuery);
		rs = ps.executeQuery();
...
private class ItemAddListener implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		quantityWarning(); // this method gives warning if the value quantity of item is below 1
...
private class ItemLists implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		// customerTable();
		ItemClass1 items = new ItemClass1(table, model, customerAddrField, customerField);
...
public void customerDetails() {
	...
	if (customerField.getText().toString().isEmpty() || customerAddrField.getText().toString().isEmpty()
			|| idField.getText().toString().isEmpty()) {
		JOptionPane.showMessageDialog(null, "Customer name / address Field is empty");
		...
		try {
			...
			// ps = DbConnection.getConnection().prepareStatement(tableName);
			// ps.execute();
...
private void quantityWarning() {
	PreparedStatement ps = null;
	String qry = "Select Quantity From ProductCategories where ProductName=?";
	String productName = (String) productBox.getSelectedItem();
	...
	try {
		...
		ResultSet rs = ps.executeQuery();
		while (rs.next()) {
			String result = rs.getString("Quantity");
			int qtty = Integer.parseInt(result);
			if (qtty == 0) {

				JOptionPane.showMessageDialog(null, "The quantity of items You selected is less than one\n"
						+ "Update the the depleted quantity");
			}
		}
	} catch (SQLException exc) {
		exc.printStackTrace();
